...
Each model is published as a versioned asset in the registry, with one or more deployment templates that pin the serving engine, the configuration, and the accelerator it runs on. Browse it in the [catalog](./catalog/index), where every entry links to the latest version of the model on Microsoft Foundry.
...
- Connect a deployment to an agent harness such as [Codex](./harness/integrations/codex) or [Hermes Agent](./harness/integrations/hermes-agent).
