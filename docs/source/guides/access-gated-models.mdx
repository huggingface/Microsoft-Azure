...
Some Hugging Face models in the Microsoft Foundry model catalog are **gated**, meaning you must request access from the model publisher on the Hugging Face Hub before you can deploy them. The access request is then tied to your Hugging Face identity.
...
When you open a model’s listing on Microsoft Foundry catalog, look at the **Details** section. If the model is gated, you will see a banner at the top mentioning a **Gated Model Access** is required.

## 2. Request access on Hugging Face

From the model’s catalog page, follow the link to the model page on the Hugging Face Hub. Click the **Request access** / **Agree and access repository** action (wording varies by model). Finally, you need to wait for approval from the model authors. Some models approve instantly; others require review that can take up to a day.
